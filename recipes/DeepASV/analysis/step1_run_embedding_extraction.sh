...
# Step 1: 用 LoRA_Adapter_MFA 模型从音频中提取说话人嵌入
# 支持多 GPU 并行处理（通过文件分片）
...
OUTPUT_DIR="${BASE}/embeddings_w2vbert_lora_alldatasets/embeddings_utterances"
NUM_GPUS=4
PROCS_PER_GPU=8
NUM_WORKERS=8
SKIP_EXISTING=true
...
echo -e "${BLUE}=== 说话人嵌入提取 (${NUM_GPUS} GPU x ${PROCS_PER_GPU} proc = ${TOTAL_PROCS} total) ===${NC}"
...
if [ "$FAILED" -eq 1 ]; then
    echo -e "${RED}部分进程失败，查看 /tmp/emb_w2v_gpu*_p*.log${NC}"
    exit 1
fi
